...
double pi_monte_carlo(long num_trials){
    long i;  long Ncirc = 0;
    double pi, x, y, test;
    double *vX = new double[num_trials];
    double *vY = new double[num_trials];
    ...
    #pragma omp parallel for reduction (+:Ncirc) private (x,y,test) //com o private, cada thread tem o seu x, y e test
        for(i=0;i<num_trials; i++){
            if (vX[i]*vX[i] + vY[i]*vY[i] <= r*r) Ncirc++;
        }
